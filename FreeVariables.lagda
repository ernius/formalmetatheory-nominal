...
--
-- lemmafv* : ∀ {a} {M} → Pfv* a M
-- lemmafv* {a} {M} = TermαIndPerm (Pfv* a) (αCompatiblePfv* a) lemmav lemma· {!!} M
--   where
...
--   lemmaƛ = {!!} --  [ a ] , lemmaƛaux
--     where 
--     lemmaƛaux : (M : Λ) (b : ℕ) → b ∉' [ a ] → (∀ c → Pfv* a (（ b ∙ c ） M)) → Pfv* a (ƛ b M)
...
--       = *ƛ (lemma*swap←≢ (sym≢ b≢a) a≢c (f c a∈fv（bc）M)) b≢a  
--       where
--       b≢a : b ≢ a
